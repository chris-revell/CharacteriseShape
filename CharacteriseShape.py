...
def findLongestRoute(filename):
    # Create black and white image
    originalimage,bw = read_cell(filename)
...
zfun = np.poly1d(z)
zs = zfun(xs)
ax[1,1].plot(xs,zs,label="Quadratic")


fitter = modeling.fitting.LevMarLSQFitter()
model = modeling.models.Gaussian1D()   # depending on the data you need to give some initial values
fitted_model = fitter(model, xs, ys)
ax[1,1].plot(xs,fitted_model(xs),label="Gaussian")
...
ax[0,1].tick_params(axis='y', which='both', left=False, right=False, labelleft=False)
